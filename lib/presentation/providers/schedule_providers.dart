...
final todaySchedulesProvider = StreamProvider.autoDispose<List<Schedule>>((
  ref,
) {
...
final customerByIdProvider = StreamProvider.autoDispose
...
final locationByIdProvider = StreamProvider.autoDispose
    .family<Location?, String>((ref, id) {
      final db = ref.watch(appDatabaseProvider);
      return (db.select(
        db.locations,
...
final ticketByIdProvider = StreamProvider.autoDispose.family<Ticket?, String>((
  ref,
  id,
) {
  final db = ref.watch(appDatabaseProvider);
...
final draftReportsProvider = StreamProvider.autoDispose<List<DraftReport>>((
  ref,
) {
  final db = ref.watch(appDatabaseProvider);
...
final allMaterialiProvider = StreamProvider.autoDispose<List<MaterialiData>>((
  ref,
) {
  final db = ref.watch(appDatabaseProvider);
...
final allColleaguesProvider = StreamProvider.autoDispose<List<Colleague>>((
  ref,
) {
  final db = ref.watch(appDatabaseProvider);
  return (db.select(
    db.colleagues,
  )..orderBy([(c) => OrderingTerm.asc(c.displayName)])).watch();
});
...
final colleagueNameProvider = StreamProvider.autoDispose
    .family<String?, String>((ref, userId) {
      final db = ref.watch(appDatabaseProvider);
      return (db.select(db.colleagues)..where((c) => c.id.equals(userId)))
          .watchSingleOrNull()
          .map((c) => c?.displayName);
    });
...
final locationNameProvider = StreamProvider.autoDispose.family<String?, String>(
  (ref, locationId) {
    final db = ref.watch(appDatabaseProvider);
    return (db.select(db.locations)..where((l) => l.id.equals(locationId)))
        .watchSingleOrNull()
        .map((l) => l?.name);
  },
);
...
final customerNameProvider = StreamProvider.autoDispose.family<String?, String>(
  (ref, customerId) {
    final db = ref.watch(appDatabaseProvider);
    return (db.select(db.customers)..where((c) => c.id.equals(customerId)))
        .watchSingleOrNull()
        .map((c) => c?.companyName);
  },
);
...
final materialeNameProvider = StreamProvider.autoDispose
    .family<String?, String>((ref, materialeId) {
      final db = ref.watch(appDatabaseProvider);
      return (db.select(db.materiali)..where((m) => m.id.equals(materialeId)))
          .watchSingleOrNull()
          .map((m) => m?.name);
    });
...
final allCantieriProvider = StreamProvider.autoDispose<List<CantieriData>>((
  ref,
) {
  final db = ref.watch(appDatabaseProvider);
  return (db.select(
...
final scheduleAssigneesProvider = StreamProvider.autoDispose
    .family<List<ScheduleAssignee>, String>((ref, scheduleId) {
      final db = ref.watch(appDatabaseProvider);
      return (db.select(
...
final teamAssignedScheduleIdsProvider = StreamProvider.autoDispose<Set<String>>(
  (ref) {
    final db = ref.watch(appDatabaseProvider);
...
final scheduleAssigneeMapProvider =
