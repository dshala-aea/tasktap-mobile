...
final customerDetailProvider = StreamProvider.autoDispose.family<Customer?, String>((ref, id) {
  final db = ref.watch(appDatabaseProvider);
...
final ticketCountForCustomerProvider = StreamProvider.autoDispose.family<int, String>((
  ref,
  customerId,
) {
  final db = ref.watch(appDatabaseProvider);
  return (db.select(
    db.tickets,
  )..where((t) => t.customerId.equals(customerId))).watch().map((rows) => rows.length);
});

/// All tickets for a given [customerId], most-recent first.
final ticketsForCustomerProvider = StreamProvider.autoDispose.family<List<Ticket>, String>((
  ref,
  customerId,
) {
  final db = ref.watch(appDatabaseProvider);
...
final clienteOverviewProvider = FutureProvider.autoDispose.family<ClienteOverviewDto, String>((
  ref,
  customerId,
) async {
  ...
  return ref.watch(clienteOverviewApiClientProvider).getOverview(customerId);
});
