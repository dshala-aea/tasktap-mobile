...
final pendingTicketsProvider = StreamProvider.autoDispose<List<PendingTicket>>((ref) {
...
final ticketStatusMapProvider = StreamProvider.autoDispose<Map<int, String>>((ref) {
  final db = ref.watch(appDatabaseProvider);
  return db.select(db.ticketStatuses).watch().map((rows) => {for (final r in rows) r.id: r.name});
});

/// Map of typeId → type name from cached TicketTypes table.
final ticketTypeMapProvider = StreamProvider.autoDispose<Map<int, String>>((ref) {
  final db = ref.watch(appDatabaseProvider);
  return db.select(db.ticketTypes).watch().map((rows) => {for (final r in rows) r.id: r.name});
});

/// All schedules for a specific ticket id.
final schedulesForTicketProvider = StreamProvider.autoDispose.family<List<Schedule>, String>((
  ref,
  ticketId,
) {
  final db = ref.watch(appDatabaseProvider);
...
final ticketReportsProvider = FutureProvider.autoDispose.family<List<TicketReportSummary>, String>((
  ref,
  ticketId,
) async {
...
final ticketMaterialiProvider = FutureProvider.autoDispose.family<List<TicketMaterialeDto>, String>(
  (ref, ticketId) async {
    if (!ref.watch(isOnlineProvider)) {
      throw const TicketDetailOfflineException();
    }
...
/// disagree with.
final ticketWorklogsProvider = FutureProvider.autoDispose.family<List<TicketWorkLogDto>, String>((
  ref,
  ticketId,
) async {
  if (!ref.watch(isOnlineProvider)) {
...
final runningTicketWorklogProvider = Provider.autoDispose.family<TicketWorkLogDto?, String>((
  ref,
  ticketId,
) {
