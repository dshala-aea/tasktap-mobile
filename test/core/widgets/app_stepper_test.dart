...
void main() {
  group('AppStepper', () {
    ...
    testWidgets('completed steps before current show a check icon', (tester) async {
    ...
    testWidgets('no checks when on the first step', (tester) async {
      await tester.pumpWidget(_wrap(const AppStepper(steps: _steps, currentIndex: 0)));
      expect(find.byIcon(LucideIcons.check), findsNothing);
    });

    testWidgets('upcoming and current steps show their 1-based number', (tester) async {
      await tester.pumpWidget(_wrap(const AppStepper(steps: _steps, currentIndex: 0)));
    ...
    testWidgets('progressing to the last step checks all prior steps', (tester) async {
      await tester.pumpWidget(_wrap(const AppStepper(steps: _steps, currentIndex: 3)));
